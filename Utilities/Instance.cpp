...
std::vector<DWORD> getChildrenPure(DWORD Instance)
{
	std::vector<DWORD> childrenVector;
	DWORD childVector = *(DWORD*)(Instance + 44);
	DWORD childEnd = *(DWORD*)(childVector + 4);
	DWORD childStart = *(DWORD*)childVector;
	for ( ; childStart != childEnd; childStart += 8 )
		childrenVector.push_back(childStart);
	return childrenVector;
}
...
DWORD getInstanceClassID(DWORD Instance)
{
	return **(DWORD**)(Instance + 0xC);
}
...
RBX::DataModel::DataModel(Instance T) : Instance(T.GetAddress())
{

}

RBX::DataModel::DataModel() : Instance()
{
}
